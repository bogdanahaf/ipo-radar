...
- **Telegram `chat not found` (HTTP 400):** `TELEGRAM_CHAT_ID` does not match a chat the bot can see. Use the public username form `@YourChannel` (no `https://t.me/`), or the numeric **supergroup** id (often starts with `-100`). The bot must be a **channel administrator** with **Post Messages**. If the channel is private, the username form will not work; use the numeric id (e.g. from a bot like `@getidsbot` after you forward any post from that channel to it, or from Telegram Web channel URL patterns you trust).
